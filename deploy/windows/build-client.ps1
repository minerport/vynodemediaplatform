...
$publishArguments += "-p:VyNodeUpdateChannel=$Channel"
$publishArguments += "-p:Version=$Version"
$publishArguments += "-p:VyNodeUpdateMetadataUrl=https://updates.vynode.app/windows/$Channel/manifest.json"
$publishArguments += "-p:VyNodeUpdateSignatureUrl=https://updates.vynode.app/windows/$Channel/manifest.sig"
...
dotnet build (Join-Path $PSScriptRoot 'installer\Desktop\VyNode.Desktop.wixproj') -c $Configuration -p:PackageVersion=$Version -p:ClientPublish=$publish -o $installer
