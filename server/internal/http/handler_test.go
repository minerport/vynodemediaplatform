...
func handler(ready error) http.Handler {
	return NewHandler(slog.New(slog.NewTextHandler(io.Discard, nil)), readyStub{ready}, SystemInfo{Version: "0.1.0", Commit: "test", OS: "testos", Architecture: "testarch", InstanceID: "instance", ServerName: "Test", DatabaseType: "sqlite", StartedAt: time.Now()}, nil, nil, nil, "")
}
...
func TestSPAHandlerServesAssetsAndProtectedRouteFallback(t *testing.T) {
	...
	h := NewHandler(slog.New(slog.NewTextHandler(io.Discard, nil)), readyStub{}, SystemInfo{WebDir: dir, StartedAt: time.Now()}, nil, nil, nil, "")
...
func TestOriginAndSecurityPolicy(t *testing.T) {
	h := NewHandler(slog.New(slog.NewTextHandler(io.Discard, nil)), readyStub{}, SystemInfo{StartedAt: time.Now()}, nil, nil, nil, "https://media.example")
