...
Phase 12 adds a separate offline domain: requests create device assignments; immutable assets represent original, optimized, or generated bytes; jobs prepare only generated MP4/H.264/AAC derivatives; subscriptions maintain episode windows; and durable per-device change cursors reconcile inventory and progress. None of these are playback sessions. `/downloads` is an explicit writable, application-owned cache root.
